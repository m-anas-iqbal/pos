                    ...
                    if (dataGridView1.RowCount > 0)
                    {
                        ...
                        if (msg == DialogResult.Yes)
                        {
                            ...
                            for (i = 0; i <= dataGridView1.RowCount - 1; i++)
                            {
                                ...
                                double Weight;
                                try { Weight = clsUtility.num_repl(dataGridView1.Rows[i].Cells["Weight_per_piece"].Value.ToString()); }
                                catch { Weight = 0; }
                                ...
                                try { Promotiontype = dataGridView1.Rows[i].Cells["Promotiontype"].Value.ToString(); }
                                catch { Promotiontype = ""; }

                                string Weeks;
                                try { Weeks = dataGridView1.Rows[i].Cells["Weeks"].Value.ToString(); }
                                catch { Weeks = ""; }
                                ...
                                clsUtility.ExecuteSQLQuery(" INSERT INTO ItemInformation (ItemName,  UnitOfMeasure, Batch,  GROUP_ID,  Barcode,  Cost,  Price,  ReorderPoint, VAT_Applicable,  WarehouseID,Weight_per_piece,Weeks,Promotiontype) VALUES " +
                                                           " ('" + ItemName + "',  '" + UnitOfMeasure + "', '" + Batch + "',  '" + GROUP_ID + "',  '" + Barcode + "',  '" + Cost + "',  '" + Price + "',  '" + ReorderPoint + "', '" + VAT_Applicable + "',  '" + Weight + "',  '" + Weeks + "',  '" + Promotiontype + "') ");
